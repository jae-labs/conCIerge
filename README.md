...
<a href="go.mod"><img src="https://img.shields.io/github/go-mod/go-version/jae-labs/conCIerge?filename=go.mod" alt="Go Version"></a>
...
This repository contains the dedicated Go Slack Bot. The Ansible host configurations have been moved to their own dedicated [jae-labs/ansible](https://github.com/jae-labs/ansible) repository. The actual Terraform IaC codebase is managed in the dedicated [iac](https://github.com/jae-labs/iac) repository.
...
## Ansible Host Configuration (OCI)

The Ansible host configurations for provisioning and deploying the bot to Oracle Cloud Infrastructure (OCI) have been moved to their own dedicated repository: **[jae-labs/ansible](https://github.com/jae-labs/ansible)**.

Please refer to the `jae-labs/ansible` repository for instructions on:
- OCI host configuration and dynamic inventory usage.
- Bot service deployment, Docker installation, nginx, certbot, and telemetry setup.
- Running deployment/management playbooks:
  ```sh
...
The bot follows a unified GitHub Actions workflow model.

| Workflow | Trigger | Behavior |
|---|---|---|
| `ci.yml` | Pushes to `main` and pull requests | Runs lint, test, cross-platform build, and security scan jobs for the bot |
| `release.yml` | Pushes to `main` | Builds Linux and macOS release artifacts, creates the next patch release, refreshes the `latest` release, and deploys the released Linux amd64 binary to production via Ansible |
