...
func DopplerBranchName(action, projectName string) string {
	...
	sanitized = nonAlphaNum.ReplaceAllString(sanitized, "")
	suffix := time.Now().Format("20060102-150405")
	return fmt.Sprintf("concierge/%s-doppler-%s-%s", action, sanitized, suffix)
}
...
func BuildDopplerPRDescription(action, projectName, requester, justification string) string {
	...
	return fmt.Sprintf(`## %s Doppler Project
...
### Changes
- %ss Doppler project entry in `+"`terraform/doppler/locals_projects.tf`"+`

---
*Created by conCierge*`, verb, projectName, justification, requester, verb)
}
