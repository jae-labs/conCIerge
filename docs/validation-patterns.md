...
To prevent session hijacking and unauthorized bot usage via callback spoofing, interactive callbacks (`handleBlockAction`) and view submissions (`handleViewSubmission`) enforce structural security checks:

1. **User Identity Validation**: Asserts that `callback.User.ID` matches `state.UserID` (the user who originally opened the thread flow). Mismatches are rejected.
2. **Access Control Check**: Re-validates that the submitting user is authorized in the user/manager/admin lists via `h.isAuthorized()`.
