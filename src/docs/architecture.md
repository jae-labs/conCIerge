...
| Inbound Slack HTTP | `internal/slack/http.go` wraps `/slack/events` and `/health` with `otelhttp.NewHandler(...)` for request spans and HTTP metrics. |
...
| Workflow telemetry | PR creation paths emit workflow spans plus counters/histograms keyed by workflow, resource, action, and outcome. |
...
| Manager | `SLACK_MANAGER_IDS` | Initiate flows |
| Admin | `SLACK_ADMIN_IDS` | Initiate flows |

Checked via `isAuthorized()`.
