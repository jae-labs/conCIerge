...
func renderRepoEntry(repo conversation.RepoConfig) (string, error) {
	...
	data := repoTmplData{
		Name:                          repo.Name,
		Description:                   strconv.Quote(repo.Description),
		Visibility:                    strconv.Quote(repo.Visibility),
		HasIssues:                     boolStr(repo.HasIssues),
		DefaultBranch:                 strconv.Quote(repo.DefaultBranch),
	...
	if repo.HomepageURL != "" {
		data.HomepageURL = strconv.Quote(repo.HomepageURL)
