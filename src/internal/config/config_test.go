...
func TestLoad_observabilityDefaults(t *testing.T) {
	...
	if cfg.OtelTracesEndpoint != "" {
		t.Errorf("got OtelTracesEndpoint=%q, want empty", cfg.OtelTracesEndpoint)
	}
...
func TestLoad_sentryDefaults(t *testing.T) {
	...
	if cfg.SentryDSN != "" {
		t.Errorf("got SentryDSN=%q, want empty", cfg.SentryDSN)
	}
	if cfg.SentryEnvironment != "production" {
		t.Errorf("got SentryEnvironment=%q, want OTEL environment fallback", cfg.SentryEnvironment)
	}
	if cfg.SentryRelease != "" {
		t.Errorf("got SentryRelease=%q, want empty", cfg.SentryRelease)
	}
}
...
func TestLoad_sentryFromEnv(t *testing.T) {
	...
	t.Setenv("SENTRY_ENVIRONMENT", "prd")
	...
	if cfg.SentryDSN != "https://public@example.invalid/1" {
		t.Errorf("got SentryDSN=%q", cfg.SentryDSN)
	}
	if cfg.SentryEnvironment != "prd" {
		t.Errorf("got SentryEnvironment=%q, want prd", cfg.SentryEnvironment)
	}
	if cfg.SentryRelease != "v1.2.3" {
		t.Errorf("got SentryRelease=%q, want v1.2.3", cfg.SentryRelease)
	}
}
...
func TestLoad_metricsListenAddrMustBeLoopback(t *testing.T) {
	setAllEnv(t)
	t.Setenv("METRICS_LISTEN_ADDR", "0.0.0.0:9100")
