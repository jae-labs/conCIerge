...
---
*Created by Opsy*`, repoName, description, justification, requester)
}
...
func MemberBranchName(action, team, username string) string {
	...
	sanitized = nonAlphaNum.ReplaceAllString(sanitized, "")
	suffix := time.Now().Format("20060102-150405")
	return fmt.Sprintf("opsy/%s-member-%s-%s", verb, sanitized, suffix)
}
...
### Changes
- Modifies team membership in `+"`iac/terraform/github/locals_members.tf`"+`

---
*Created by Opsy*`, verb, team, username, roleLine, justification, requester)
}
