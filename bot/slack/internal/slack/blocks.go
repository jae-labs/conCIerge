...
var priorityOptions = []*slack.OptionBlockObject{
	slack.NewOptionBlockObject("low", slack.NewTextBlockObject("plain_text", "\U0001F7E2 Low", true, false), nil),
	slack.NewOptionBlockObject("medium", slack.NewTextBlockObject("plain_text", "\U0001F7E1 Medium", true, false), nil),
	slack.NewOptionBlockObject("high", slack.NewTextBlockObject("plain_text", "\U0001F534 High", true, false), nil),
}

// requestPriorityBlock returns an input block for selecting request priority.
...
// priorityEmoji returns the emoji for a priority level.
...
func HomeTabBlocks(userID string) []slack.Block {
	greeting := slack.NewSectionBlock(
		slack.NewTextBlockObject("mrkdwn", fmt.Sprintf("Hi <@%s>! I'm *Opsy*, your infrastructure assistant.", userID), false, false),
		nil, nil,
	)
...
func WelcomeBlocks(userID string) []slack.Block {
	header := slack.NewSectionBlock(
		slack.NewTextBlockObject("mrkdwn", fmt.Sprintf("Hey <@%s>, I'm Opsy. Let's get things done.\nWhat would you like to set up?", userID), false, false),
		nil, nil,
	)
