...
Step-by-step guide for adding support for a new terraform resource (e.g., Doppler) to the Opsy bot.
...
```go
pathDopplerProjects = "iac/terraform/doppler/locals_projects.tf"
pathDopplerAccess   = "iac/terraform/doppler/locals_access.tf"
```
...
Follow naming pattern: `opsy/{action}-doppler-{sanitized}-{timestamp}`.
...
6. Create PR
7. Call `replyPR()` which creates Linear issue and posts summary

### Step 11: Update Linear description builder

File: `internal/slack/handler.go` -- `buildLinearIssueDescription()`

Add case for new resource type in the switch statement.

### Step 12: Update documentation

- Add bot integration status to terraform module doc (`iac/terraform/docs/doppler-module.md`)
- Update `bot/slack/AGENTS.md` cross-system contract tables
...
| 11 | `slack/handler.go` | Linear description |
| 12 | docs, AGENTS.md | Documentation |
